...
class student{
    ...
    student(const student& other) {
        rollNo = other.rollNo; // Copy the roll number
        name = new char[strlen(other.name) + 1]; // Allocate memory for the name
        strcpy(name, other.name); // Copy the name from the other student object
    }
    // setter /Getter
